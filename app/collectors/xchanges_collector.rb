...
class XchangesCollector < YQLCollector
  ...
  def query
    ...
    self.where_in(:pair, selected_pairs)
  end
  ...
  def store(results)
    ActiveRecord::Base.transaction do
      results.each do |result|
        ...
        current_xchange.pair_id = result['id'] # NOTE: 'id' is reserved by ActiveRecord.
    ...
    pairs_with_xchanges.each do |pair|
      ...
      ActiveRecord::Base.transaction do
        ...
        if ['mysql'].include?($DB_CONFIG[$ENV]['adapter']) 
          ...
          Xchange.import [:id, :change], mapped_updated_xchanges, on_duplicate_key_update: [:id]
        end
        OrderedKey.delete_all
      end
    end
