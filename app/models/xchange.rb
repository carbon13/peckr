...
class Xchange < ActiveRecord::Base
  ...
  def current(target_pair_id)
    Xchange.where{ pair_id == target_pair_id }.ordered_desc.first
  end

  def previous(self_pair_id = pair_id, self_date = date, self_time = time, self_created_at = created_at) 
  ...
  def next(self_pair_id = pair_id, self_date = date, self_time = time, self_created_at = created_at)
  ...
  def registered?(self_pair_id = pair_id, self_rate = rate, self_date = date, self_time = time)
    if Xchange.find_by(pair_id: self_pair_id).nil?
      false
    else
      !!Xchange.find_by(pair_id: self_pair_id, rate: self_rate, date: self_date, time: self_time)
    end
  end
