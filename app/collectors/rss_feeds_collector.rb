...
class RssFeedsCollector
  ...
  def store(results)
    results.each do |rss_source_link, result|
      result.xpath('//channel').each do |node|
        ...
        rss_source.title = node.xpath('./title').text
        # FIXME: rss_source.atom_link = node.xpath('./atom:link').attribute('href').value
        rss_source.description = node.xpath('./description').text
        rss_source.last_build_date = Time.rfc2822(node.xpath('./lastBuildDate').text)
        rss_source.language = node.xpath('./language').text
        rss_source.generator = node.xpath('./generatorTime').text
        rss_source.save
      end
      result.xpath('//item').each do |node|
        unless RssFeed.find_by(title: node.xpath('.//title').text)
          ...
          rss_feed.pubdate = Time.rfc2822(node.xpath('.//pubDate').text)
          rss_feed.save 
        end
      end
    end
  ...
  def store_bodies(results)
    results.each do |rss_feed_id, result|
      exclusive_tag_names = ['script', 'head']
